...
Config.Staff.Group = {
    ...
    ['administrateur'] = '~p~Administrateur~s~',
...
Config.eventsMenu = {
    ["Braquage Véhicule (Attaque)"] = {
        ...
        event = function(result)
            TriggerServerEvent('zmain:admin:vents:van:server:create', result.position, result.heading, result.rewards, result.duration, result.vehicle or 'stockade', result.name or "Fourgon Blindé")
        end,
...
Config.Staff.HavePermission = function(permissionsCategorie, permissionName, player)
    ...
    if not player then
        ...
        elseif Config.Staff.Permissions[permissionsCategorie][permissionName][ESX.GetPlayerData()['group']] then
            return true
        else
            return false
        end
