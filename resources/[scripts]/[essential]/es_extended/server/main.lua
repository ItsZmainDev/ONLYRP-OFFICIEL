...
function LoadUser(source, identifier)
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT UniqueID, playerName, permission_group, permission_level, accounts, job, job_grade, job2, job2_grade, firstname, lastname, sex, dateofbirth, inventory, loadout, position, rank FROM users WHERE identifier = @identifier', {
...
AddEventHandler('playerDropped', function(reason)
	...
	if xPlayer then
		...
		local zmainConnected = 0
		MySQL.Async.execute('UPDATE users SET connected = @connected, ip = @ip WHERE identifier = @identifier', {
			['@connected'] = zmainConnected,
			['@identifier'] = xPlayer.identifier,
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			if targetXPlayer.canCarryItem(itemName, itemCount) then
				...
				sourceXPlayer.showAdvancedNotification("Only\'RP", "~y~Inventaire", _U('gave_item', itemCount, ESX.Items[itemName].label, targetXPlayer.name), 'icon', 7)
				targetXPlayer.showAdvancedNotification("Only\'RP", "~y~Inventaire", _U('received_item', itemCount, ESX.Items[itemName].label, sourceXPlayer.name), 'icon', 7)

				TriggerEvent("esx:giveitemalert", sourceXPlayer.name, targetXPlayer.name, itemName, itemCount)
			else
				sourceXPlayer.showAdvancedNotification("Only\'RP", "~y~Inventaire", _U('ex_inv_lim', targetXPlayer.name), 'icon', 7)
			end
		else
			sourceXPlayer.showAdvancedNotification("Only\'RP", "~y~Inventaire", _U('imp_invalid_quantity'), 'icon', 7)
		end
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			sourceXPlayer.showAdvancedNotification("Only\'RP", "~y~Portefeuille", _U('gave_account_money', ESX.Math.GroupDigits(itemCount), accountLabel, targetXPlayer.name), 'icon', 9)
			targetXPlayer.showAdvancedNotification("Only\'RP", "~y~Portefeuille", _U('received_account_money', ESX.Math.GroupDigits(itemCount), accountLabel, sourceXPlayer.name), 'icon', 9)

			TriggerEvent("esx:giveaccountalert", sourceXPlayer.name, targetXPlayer.name, itemName, itemCount)
		else
			sourceXPlayer.showAdvancedNotification("Only\'RP", "~y~Portefeuille", _U('imp_invalid_amount'), 'icon', 9)
		end
	elseif type == 'item_weapon' then
		...
		if sourceXPlayer.hasWeapon(itemName) then
			...
			if not targetXPlayer.hasWeapon(itemName) then
				...
				if itemCount > 0 then
					sourceXPlayer.showAdvancedNotification("Only\'RP", "~y~Armes", _U('gave_weapon_withammo', weaponLabel, itemCount, targetXPlayer.name), 'icon', 7)
					targetXPlayer.showAdvancedNotification("Only\'RP", "~y~Armes", _U('received_weapon_withammo', weaponLabel, itemCount, sourceXPlayer.name), 'icon', 7)
				else
					sourceXPlayer.showAdvancedNotification("Only\'RP", "~y~Armes", _U('gave_weapon', weaponLabel, targetXPlayer.name), 'icon', 7)
					targetXPlayer.showAdvancedNotification("Only\'RP", "~y~Armes", _U('received_weapon', weaponLabel, sourceXPlayer.name), 'icon', 7)

					TriggerEvent("esx:giveweaponalert", sourceXPlayer.name, targetXPlayer.name, itemName)
				end
			else
				sourceXPlayer.showAdvancedNotification("Only\'RP", "~y~Armes", _U('gave_weapon_hasalready', targetXPlayer.name, weaponLabel), 'icon', 7)
				targetXPlayer.showAdvancedNotification("Only\'RP", "~y~Armes", _U('received_weapon_hasalready', sourceXPlayer.name, weaponLabel), 'icon', 7)
			end
		end
...
AddEventHandler('esx:dropInventoryItem', function(type, itemName, itemCount)
	...
	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			xPlayer.showAdvancedNotification("Only\'RP", "~y~Inventaire", _U('imp_invalid_quantity'), 'icon', 7)
		else
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				xPlayer.showAdvancedNotification("Only\'RP", "~y~Inventaire", _U('threw_standard', itemCount, ESX.Items[itemName].label), 'icon', 7)
			end
		end
	elseif type == 'item_account' then
		if itemCount == nil or itemCount < 1 then
			xPlayer.showAdvancedNotification("Only\'RP", "~y~Portefeuille", _U('imp_invalid_amount'), 'icon', 9)
		else
			...
			if (itemCount > account.money or account.money < 1) then
				...
				xPlayer.showAdvancedNotification("Only\'RP", "~y~Portefeuille", _U('threw_account', ESX.Math.GroupDigits(itemCount), string.lower(accountLabel)), 'icon', 9)
			end
		end
	elseif type == 'item_weapon' then
		itemName = string.upper(itemName)
		...
		if xPlayer.hasWeapon(itemName) then
			...
			if weapon.ammo > 0 then
				xPlayer.showAdvancedNotification("Only\'RP", "~y~Armes", _U('threw_weapon_ammo', weapon.label, weapon.ammo), 'icon', 7)
			else
				xPlayer.showAdvancedNotification("Only\'RP", "~y~Armes", _U('threw_weapon', weapon.label), 'icon', 7)
			end
		end
	end
end)
	...
	if itemData.type ~= 'item_clothes' then
		if xItem then
			if xItem.count > 0 then
				...
				xPlayer.showAdvancedNotification("Only'RP", "~y~Inventaire", _U('act_imp'), 'icon', 7)
			end	
		end
	
		if xItem2 then
			TriggerClientEvent('zmain:framework:sortWeapon', source, itemName)
		end
	else
...
RegisterNetEvent('zmain:framework:savePlayers', function(source)
	ESX.SavePlayers()
end)

RegisterNetEvent('esx:restartFramework', function()
	local source = source
	--TriggerEvent("ratelimit", source, "esx:firstJoinProper")
