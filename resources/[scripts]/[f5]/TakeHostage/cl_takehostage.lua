...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Tu as besoin d'une armes chargée pour prendre en otage!")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			drawNativeNotification("Il n'y as personne pour prendre en otage!")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"[G] pour relacher, [H] pour tuer")
