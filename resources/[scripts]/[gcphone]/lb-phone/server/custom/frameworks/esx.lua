CreateThread(function()
    ...
    function GetIdentifier(source)
        local GetPlayerIdentifier = ESX.GetPlayerFromId(source)
        if type(GetPlayerIdentifier) == "table" then
            return GetPlayerIdentifier.identifier
        end
    end
    ...
    RegisterCommand("toggleverified", function(source, args)
        local xPlayer = ESX.GetPlayerFromId(source)
        if xPlayer.getGroup() ~= 'fondateur' then return end
        local app, username, verified = args[1], args[2], tonumber(args[3])
        if app ~= "twitter" and app ~= "instagram" and app ~= 'tiktok' then
            return print("No such app " .. tostring(app))
        end
        if not username then
            return print("No username provided")
        end
        if verified ~= 1 and verified ~= 0 then
            return print("Verified must be 1 or 0")
        end
