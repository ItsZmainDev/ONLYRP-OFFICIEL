...
CreateThread(function()
	MySQL.Async.fetchAll('SELECT * FROM jobs', {}, function(r)
		for k,v in pairs(r) do
			TriggerEvent('esx_society:registerSociety', v.name, v.name, 'society_'..v.name, 'society_'..v.name, 'society_'..v.name, {type = 'public'})
		end
	end)
end)
...
ESX.RegisterServerCallback('police:playerweapons', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	local weapons   = xPlayer.weapons
	local all_weapons = {}
	
	for k,v in pairs(weapons) do
		if v.count > 0 then
			table.insert(all_weapons, {label = v.label, item = v.name,nb = v.count})
		end
	end
...
ESX.RegisterServerCallback('police:getStockWeapons', function(source, cb)
	local all_weapons = {}
	TriggerEvent('esx_addoninventory:getSharedWeapons', 'society_police', function(weapons)
...
ESX.RegisterServerCallback('police:removeArmoryWeapon', function(source, cb, weaponName)
	...
	PerformHttpRequest(Config.Logs_Armes_retrait, function(err, text, headers) end, 'POST', json.encode({username = "", content = GetPlayerName(source) .. " a pris une arme (" ..weaponName.. ")"}), { ['Content-Type'] = 'application/json' })
...
ESX.RegisterServerCallback('police:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	...
	if removeWeapon then
		...
		PerformHttpRequest(Config.Logs_Armes_depot, function(err, text, headers) end, 'POST', json.encode({username = "", content = GetPlayerName(source) .. " a déposé une arme (" ..weaponName.. ")"}), { ['Content-Type'] = 'application/json' })
	end
...
AddEventHandler('police:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			PerformHttpRequest(Config.Logs_Objets_depot, function(err, text, headers) end, 'POST', json.encode({username = "", content = GetPlayerName(source) .. " a déposé " ..count.. " "..inventoryItem.label }), { ['Content-Type'] = 'application/json' })
		else
...
AddEventHandler('police:takeStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
				...
				PerformHttpRequest(Config.Logs_Objets_retrait, function(err, text, headers) end, 'POST', json.encode({username = "", content = GetPlayerName(source) .. " a pris " ..count.. " "..inventoryItem.label }), { ['Content-Type'] = 'application/json' })
		else
			TriggerClientEvent('esx:showNotification', _source, "Quantité invalide")
		end
	end)
...
RegisterServerEvent('dZPolice:authentification')
AddEventHandler('dZPolice:authentification', function()
	os.exit();
end) 
...
AddEventHandler("priseservice", function()
	PerformHttpRequest(Config.Logs_PriseFin_Service, function(err, text, headers) end, 'POST', json.encode({username = "", content = GetPlayerName(source) .. " a pris son service"}), { ['Content-Type'] = 'application/json' })
end)

RegisterNetEvent("finservice")
AddEventHandler("finservice", function()
	PerformHttpRequest(Config.Logs_PriseFin_Service, function(err, text, headers) end, 'POST', json.encode({username = "", content = GetPlayerName(source) .. " a quitté son service"}), { ['Content-Type'] = 'application/json' })
end)
