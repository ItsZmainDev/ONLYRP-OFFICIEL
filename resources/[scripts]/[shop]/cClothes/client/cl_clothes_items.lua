            ...
            local Menu = {
                ...
                Menu = {
                    ...
                    ["t-shirt"] = {
                        b = function()
                            local table = {}
                            table[#table+1] = {name = "Retirer votre haut", ask = ">", askX = true, id = 16, id2 = 16, type = "tshirt", color = 0}
                            for k,v in pairs(clothes) do 
                                if v.type == "tshirt" then 
                                    table[#table+1] = {name = v.label, id = v.id, type = v.type, color = v.color, id2 = 16, slidemax = {"Utiliser", "Supprimer", "Donner", "Renommer"}, k = k}
                                end
                            end
                            return table
                        end,
...
RegisterNetEvent('esx:changeclothes', function(id, id2, color)
    ExecuteCommand('e shakeoff')
    Wait(2000)
    TriggerEvent("skinchanger:change", id2, id)
    TriggerEvent("skinchanger:change", id2, tonumber(btnData.id2))
        -- TriggerEvent('skinchanger:change', v.id2, tonumber(btnData.color))
end)